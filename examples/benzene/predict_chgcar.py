...
jl = JLPredictor(jl_settings=settings, model_path=f"scikit_{model_name}.p", grid_size=(180,180,180))
...
for frame in tqdm(range(nframes)):
    ...
    t_init = time.time()
    ml_chg = jl.predict_chgcar(atoms,nelect=30,batch_size=batch_size,verbose=False,save_path=save_path,name=f'CHGCAR_test_{frame}',return_chg=True,write_chgcar=True)
